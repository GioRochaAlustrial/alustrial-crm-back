...
export async function createCita({
  ...
  const sql = `
    INSERT INTO citas (
      id_prospectos, id_usuarios, fecha_hora, nota, tipo, estado, realizada_por,
      categoria, auth_estado, auth_by, auth_at, auth_motivo,
      created_at
    )
    VALUES ($1,$2,$3,$4,$5,$6,NULL,$7,$8,NULL,NULL,NULL,NOW())
    RETURNING *;
  `;
  ...
  const values = [
    ...
    String(auth_estado).toUpperCase(),
  ];
...
export async function listAutorizacionesPendientesByDeptos({ deptos, categoria }) {
  ...
  if (deptos?.length) {
    filters.push(`c.tipo = ANY($${i++}::text[])`);
