...
export async function resolverAutorizacionCita({ id_cita, accion, auth_by, motivo = null }) {
  ...
  const sql = `
    ...
    SET
      ...
      auth_motivo = $3,
      estado = CASE
        WHEN $4 = 'RECHAZAR' THEN 'CANCELADA'
        ELSE estado
      END,
      updated_at = NOW()
    WHERE id = $5
...
export async function listAutorizacionesByCategoria({ categoria, auth_estado }) {
  const values = [];
  let i = 1;
  const filters = [];
  ...
  if (auth_estado && auth_estado !== "TODAS") {
    filters.push(`c.auth_estado = $${i++}`);
    values.push(String(auth_estado).toUpperCase());
  }
